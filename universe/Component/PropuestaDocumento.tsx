...
function PropuestaDocumento({ id, tituloDocumento, descripcionDocumento }: Props): JSX.Element {
	const aceptar = async (idDocumento: number) => {
		try {
			const res = await fetch(
				"http://localhost:3333/api/community/" + localStorage.getItem("comunidad_ID") + "/accept_document",
				{
					method: "POST",
					headers: {
	...
	const rechazar = async (idDocumento: number) => {
		try {
			const res = await fetch(
				"http://localhost:3333/api/community/" + localStorage.getItem("comunidad_ID") + "/reject_document",
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
