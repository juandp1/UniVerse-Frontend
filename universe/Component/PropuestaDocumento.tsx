...
function PropuestaDocumento({id,tituloDocumento, descripcionDocumento}:Props):JSX.Element{
    ...
    const aceptar = async (idDocumento:number) => {
        try {
            const res = await fetch('http://localhost:3333/api/community/'+localStorage.getItem("comunidad_ID")+'/accept_document', {
                method: 'POST',
                headers: {
    ...
    const rechazar = async (idDocumento:number) => {
        try {
            const res = await fetch('http://localhost:3333/api/community/'+localStorage.getItem("comunidad_ID")+'/reject_document' , {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${Cookies.get('token')}`
            ...
            <div className={style.square_documento}>
                ...
                <div className={style.minisquare_documento}>
                    <div className="flex justify-center space-x-10" style={{ gap:'150px', marginTop: '33px'}}>
                        <button className={style.botones} onClick={()=> aceptar(id)} >Aceptar</button>
                        <button className={style.botones} onClick={()=> rechazar(id)} >Rechazar</button>
                    </div>
                </div>
