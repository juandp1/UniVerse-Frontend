...
export default function PestaniaComunidad() {
    ...
    useEffect(() => {
        const fetchData = async () => { // se trae la informacion de las comunidades que existen al entrar a la pagina
            //setIsLoading(true)
            try {
                const res = await fetch("http://127.0.0.1/api/communities", {
                    method: 'GET',
                    mode: 'cors',
                    headers: {
    ...
    useEffect(() => {
        const fetchData = async () => { // se trae la informacion de los labels que existen al entrar a la pagina
            //setIsLoading(true)
            try {
                const res = await fetch("http://127.0.0.1/api/labels", {
                    method: 'GET',
                    mode: 'cors',
                    headers: {
                        'Content-Type': 'application/json'
    ...
    const abandonarComunidad = async () => {
        try {
            const res = await fetch('http://127.0.0.1/api/leave_community', {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const crearComunidad = async (values: Comunidad) => {
        ...
        try {
            const res = await fetch('http://127.0.0.1/api/community', {
                method: 'POST',
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const updateComunidad = async (values: Comunidad) => {
        try {
            const res = await fetch('http://127.0.0.1/api/community/name/' + comunityName, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${Cookies.get('token')}`
    ...
    const buscarComunidad = async (name: string) => {
        if (name === "" || name == null) {
            ...
            try {
                const res = await fetch('http://127.0.0.1/api/community/similar_name/' + name, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${Cookies.get('token')}`
