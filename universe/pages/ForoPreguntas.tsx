...
export default function ForoPreguntas() {
    ...
    useEffect(() => {
        const fetchData = async () => { // se trae la información de las preguntas que existen al entrar a la página./api/community/<int:community_id>/topic/<int:topic_id>/questions
            try {
                const res = await fetch('http://127.0.0.1/api/community/' + localStorage.getItem("comunidad_ID") + '/topic/' + localStorage.getItem("Topic")+ '/questions', {
                    method: 'GET',
                    headers: {
    ...
    const CrearPregunta = async (values: Pregunta) => {
        ...
        try {
            const res = await fetch('http://127.0.0.1/api/questions', {
                method: 'POST',
                mode: 'cors',
                headers: {
                    'Content-Type': 'application/json',
